...
my_actuator_control::my_actuator_control(ros::NodeHandle nh)
{
    ...
    motor_ids.reserve(4);
     ...
     forword_direction = 0; // if -ve mean backword and +ve mean forword 
     angular_direction = 0; // if -ve mean antiClock wise and +ve mean clock_wise
    ...
    //  odometer_timer_ = nh_.createTimer(ros::Duration(0.1), &my_actuator_control::get_odometer, this);
     voltage_timer_ = nh_.createTimer(ros::Duration(0.1), &my_actuator_control::get_voltage, this);
    ...
    // odometry_timer_ = nh_.createTimer(ros::Duration(0.1), &my_actuator_control::get_odometry, this);
                                                                                        // healthinfo_pub_timer_ = nh_.createTimer(ros::Duration(10), &my_actuator_control::get_health_info, this);
...
//         bag4.write("motor_state4", ros::srcTime::now(), motor_state4_msg);
//     } else {
...
void my_actuator_control::init_subs_pubs_srvs()
{
  ...
  //  motor_state4_publisher_ = nh_.advertise<std_msgs::Int16MultiArray>("motor_state4", 10);
   ...
   voltage_sub_ = nh_.subscribe("/voltage", 10, &my_actuator_control::voltageCallback, this);
    ...
    reset_odom_srv_ = nh_.advertiseService("/reset_odom", &my_actuator_control::reset_odometry_service_callback, this);
...
bool my_actuator_control::reset_odometry_service_callback(std_srvs::TriggerRequest &req, std_srvs::TriggerResponse &res){
...
void my_actuator_control::joy_callback(const geometry_msgs::Twist &msg)
{
    ...
    if(vel.linear.x>0){
        forword_direction = 1;
       ROS_INFO("linear_direction forword %d ",forword_direction);
    }
    else if(vel.linear.x<0){
        forword_direction = -1;
        ROS_INFO("linear_direction backword %d ",forword_direction);
    }
    else{
        forword_direction = 0;
        ROS_INFO("linear_direction  %d ",forword_direction);
    }
     
    //ROS_INFO("forword_direction forword %d ",forword_direction);
    ...
    if(vel.angular.z>0){
        angular_direction = 1;
        ROS_INFO("angular_direction clock_wise %d ",angular_direction);
    }
    else if(vel.angular.z<0){
        angular_direction = -1;
       ROS_INFO("angular_direction anti_clock_wise %d ",angular_direction);
    }
    else{
        angular_direction = 0;
       ROS_INFO("angular_direction anti_clock_wise %d ",angular_direction);
...
//     int16_t motor_stat3[4] = {0};
//     int16_t motor_stat4[4] = {0};

    
    
//     if(init_teleop_)
//     {
//         act_api_->Motor_state2(1, motor_stat1);
//         act_api_->Motor_state2(2, motor_stat2);
//         act_api_->Motor_state2(3, motor_stat3);
//         act_api_->Motor_state2(4, motor_stat4);
...
//         int16_t velocity_left_front = std::abs(motor_stat1[2]) * 0.16667 * 3.14 * 105 / 60;
//         int16_t velocity_right_front = std::abs(motor_stat2[2]) * 0.16667 * 3.14 * 105 / 60;
//         int16_t velocity_left_rear = std::abs(motor_stat3[2]) * 0.16667 * 3.14 * 105 / 60;
//         int16_t velocity_right_rear = std::abs(motor_stat4[2]) * 0.16667 * 3.14 * 105 / 60;

//         motor_speed_left_ = (velocity_left_front + velocity_left_rear) / 2.0;
//         motor_speed_right_ = (velocity_right_front + velocity_right_rear) / 2.0;

//         speed = (motor_speed_left_ + motor_speed_right_) / 2.0 ;
//         speed_msg.data = speed;
//         speed_pub_.publish(speed_msg); 
//         //ROS_INFO("velocity_left_front Speed: %d mm/s, velocity_left_rear Speed: %d mm/s", velocity_left_front, velocity_left_rear);


//         ROS_INFO("Left Speed: %d mm/s, Right Speed: %d mm/s", motor_speed_left_, motor_speed_right_);
//     }
...
void my_actuator_control::get_voltage(const ros::TimerEvent &)
{
    ...
    act_api_->Motor_state1(2, motor_stat1);
...
/*void my_actuator_control::get_odometry(const ros::TimerEvent &)
{
  ...
  //last_time = ros::Time::now();
   ...
   // x += delta_x;
    ...
    velocity_left_cmd_ = ((vel.linear.x - vel.angular.z * WHEEL_BASE_ / 2.0) / WHEEL_RADIUS_) * velocity_scale_;
            ...
            x += velocity_left_cmd_ ;
    y += velocity_left_cmd_ ;
    //y += delta_y;
    th += vel.angular.z * dt;
...
//     double delta_time = (current_time - last_odom_time_).toSec();
//     last_odom_time_ = current_time;

//     double v_left = motor_speed_left_ / 1000.0; 
//     double v_right = motor_speed_right_ / 1000.0; 

//     double v_linear = ((v_left + v_right) / 2.0)*forword_direction;
//     double v_angular = (v_right - v_left) / WHEEL_BASE_;


//     double delta_theta = v_angular * delta_time;
//     theta_ += delta_theta;
    
//     theta_ = atan2(sin(theta_), cos(theta_));

//     double delta_x = v_linear * cos(theta_) * delta_time;
//     double delta_y = v_linear * sin(theta_) * delta_time;
